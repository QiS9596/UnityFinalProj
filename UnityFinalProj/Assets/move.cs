...
public class move : MonoBehaviour {
    ...
    public string scrollbarValue(string s)
    {
        return s;
    }
	...
	void Update () {
        ...
        if (speedScrollbar.value > 0.5f )
        {
            ...
            if (speed + (speedScrollbar.value - 0.5f) * 0.2f > limitSpeed)
                ...
                speed += (speedScrollbar.value - 0.5f) * 0.2f;
        ...
        else if (speedScrollbar.value < 0.5f)
        {
            ...
            if (speed - speedScrollbar.value * 0.2f < 0)
                ...
                speed -= 0.2f + speedScrollbar.value * 0.2f;//原本應該只有speedScrollbar.value * 0.2f，但會造成當拉到最下面不會減速的情況，所以多+0.2
        ...
        //移動
        transform.Translate(transform.forward * Time.deltaTime * speed*20);
        //顯示速度
        vText.text = "velocity:" +  speed * 20;
        
        //鏡頭位置
        sCamera.transform.position = transform.position - sCamera.transform.forward * 10f + sCamera.transform.up * 2.0f;
        ...
        {//啟動搖桿
            ...
            if (CnInputManager.GetAxis("Vertical") > 0 && transform.rotation.x - forwordX > -(limitRotRate / 180))
            {//向上
                transform.Rotate(new Vector3(-1, 0, 0), RotRate * CnInputManager.GetAxis("Vertical"));
            }
            if (CnInputManager.GetAxis("Vertical") < 0 && transform.rotation.x - forwordX < (limitRotRate / 180))
            {//向下
                transform.Rotate(new Vector3(1, 0, 0), RotRate * (-CnInputManager.GetAxis("Vertical"))); //print(transform.rotation.x);
            }
            if (CnInputManager.GetAxis("Horizontal") < 0 && transform.rotation.y - forwordY > -(limitRotRate / 180))
            {//向左
                transform.Rotate(new Vector3(0, -1, 0), RotRate*(-CnInputManager.GetAxis("Horizontal")));
            }
            if (CnInputManager.GetAxis("Horizontal") > 0 && transform.rotation.y - forwordY < (limitRotRate / 180))
            {//向右
                transform.Rotate(new Vector3(0, 1, 0), RotRate*CnInputManager.GetAxis("Horizontal"));
    ...
    public void turn_onClick()
    {
        ...
        //將目前方向設為前進中心方向
        sCamera.transform.position = transform.position - transform.forward * 10f + sCamera.transform.up * 2.0f; 
        sCamera.transform.rotation=transform.rotation;
